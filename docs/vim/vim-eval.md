...
* 一些重要的技术细节：`:help NL-used-for-Nul`，文件中的0字符( 0x00 )，在内存中表示为换行符( 0x0a )，更多可参考`readfile()`, `writefile()`函数的说明
...
* 列表的每一项必须为`string`或`number`，写文件时，每一项**内部**的所有`换行符( 0x0a )`替换成`NUL( 0x00 )`，每一项**之间**用`换行符( 0x0a )`分隔
* 若flags为`'b'`，最后一个列表项之后不会添加换行符；否则，最后一项也会添加换行符
* `回车符( 0x0d )`不会自动插入，若要在输出的文件中插入回车符，则需要在列表项的内容中先插入回车符，再传给writefile()
* 另外，寄存器的包含的多行内容，是用`NUL( 0x00 )`分隔的（:reg命令查看，显示成`^J`），若将寄存器内容作为`一个列表项`写到文件中，会导致获取文件内容时，发现换行丢失
...
或者**更好的办法**，先将寄存器内容切分成list，再写入文件：

    let text = getreg( 'z' )
    let list = split( text, '\%x00' )
    call writefile( list, '/tmp/vim-reg-file', 'b' )
